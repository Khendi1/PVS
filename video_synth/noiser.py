...
class ImageNoiser:
    def __init__(self, noise_type: NoiseType = NoiseType.NONE):
        self.noise_intensity = params.add("noise_intensity", 0.0, 100, 0.5)
        self.noise_type = params.add("noise_type", 0, 6, 1) # 0=None, 1=Gaussian, 2=Salt&Pepper, 3=Poisson, 4=Speckle
    ...
    def noisy(self, image):
        ...
        if self.noise_type == NoiseType.GAUSSIAN:
            ...
            image = self.salt_and_pepper_noise(image)
        ...
        elif self.noise_type == NoiseType.SPARSE:
            image = self.sparse_noise(image)
        elif self.noise_type == NoiseType.RANDOM:
    ...
    def salt_and_pepper_noise(self, image):
        ...
        image = out.astype(np.uint8)  # Convert back to uint8
    ...
    def poisson_noise(self, image):
        ...
        image_float = image.astype(np.float64)  # Convert to float for precision
    ...
    def sparse_noise(self, image):
        """
        Applies Sparse noise to the image.
